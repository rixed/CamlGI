...
let output ty fd id s ofs len =
  ...
  Bytes.set r 3 (Char.chr(id land 255)); (* requestId *)
...
(* [output_string_unsafe ty fd id s] send on [fd] for a request [id], a
   ...
   [Bytes.length s <= 65535]. *)
let output_string_unsafe ty fd id s =
  output ty fd id (Bytes.of_string s) 0 (String.length s)
...
let output_string =
  ...
  fun ty fd id s ->
    if Bytes.length s > 0 then send_range (output ty fd id) s 0 (Bytes.length s)


let send_end_request fd id status exit_code =
  let r = Bytes.create 16 in
  Bytes.set r 0 fcgi_version;
  Bytes.set r 1 fcgi_end_request;
  Bytes.set r 2 (Char.chr(id lsr 8));
  Bytes.set r 3 (Char.chr(id land 255)); (* requestId *)
  Bytes.set r 4 '\000';
  Bytes.set r 5 '\008'; (* contentLength = 8 *)
  Bytes.set r 6 '\000'; (* no padding *)
  Bytes.set r 11 (Char.chr(exit_code land 255)); (* appStatus (4 bytes) *)
  let exit_code = exit_code lsr 8 in
  Bytes.set r 10 (Char.chr(exit_code land 255));
  let exit_code = exit_code lsr 8 in
  Bytes.set r 9 (Char.chr(exit_code land 255));
  Bytes.set r 8 (Char.chr(exit_code lsr 8));
  Bytes.set r 12 (Char.unsafe_chr(match status with
                            | REQUEST_COMPLETE -> 0
                            | CANT_MPX_CONN ->    1
                            | OVERLOADED ->       2
...
let send_unknown_type fd t =
  let r = Bytes.create 16 in
  Bytes.set r 0 fcgi_version;
  Bytes.set r 1 fcgi_unknown_type;
  Bytes.set r 2 '\000';
  Bytes.set r 3 '\000'; (* requestId = 0 *)
  Bytes.set r 4 '\000';
  Bytes.set r 5 '\008'; (* contentLength = 8 *)
  Bytes.set r 6 '\000'; (* no padding *)
...
let padding_buffer = Bytes.create 256
...
(* [get_length data ofs] returns the length at offset [ofs] encoded
   ...
   assumed that [0 <= ofs < Bytes.length s]. *)
let get_length data ofs =
  let b = Char.code(Bytes.get data ofs) in
  if b lsr 7 = 0 then
    (b, ofs + 1)
  else begin
    if ofs + 3 >= Bytes.length data then failwith "add_params";
...
let add_params =
  let rec add data ofs datalen h =
    if ofs < datalen then begin
      ...
      let name = String.uppercase_ascii (Bytes.sub_string data ofs namelen) in
      Hashtbl.add h name (Bytes.sub_string data ofs_value valuelen);
      add data ofs_next datalen h
    end in
  fun h data -> add data 0 (Bytes.length data) h
...
(* Printf.eprintf "Begin  id = %i (%i)\n" record.rec_id (Obj.magic conn.fd); flush stderr; *)
    ...
    | '\004' -> (* FCGI_PARAMS --------------------------------------- *)
        ...
        if Bytes.length record.data > 0 then
    ...
    | '\005' -> (* FCGI STDIN ---------------------------------------- *)
        ...
        if Bytes.length record.data > 0 then
          ...
          Buffer.add_bytes request.buf record.data
        else
